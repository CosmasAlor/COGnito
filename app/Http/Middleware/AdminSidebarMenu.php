    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            if (auth()->user()->can('supplier.view') || auth()->user()->can('customer.view') || auth()->user()->can('supplier.view_own') || auth()->user()->can('customer.view_own')) {
                $menu->dropdown(('Suppliers & Clients'),
                    function ($sub) {
...
if ($user && ($user->can('daily_rate.view') || $user->can('daily_rate.create'))) {
    $menu->dropdown('Daily Rates Management', function ($sub) use ($user) {
        if ($user->can('daily_rate.view')) {
            $sub->url(
                action([\App\Http\Controllers\RateController::class, 'index']),
                'List Daily Rates',
                [
                   
                    'active' => request()->is('daily-rate')
                ]
            );
        }
    }, [
        'icon' => 'fa fa-calendar', // Only pass the class name here, not HTML
        'id'   => 'menu_daily_rate',
    ])->order(25);
}
...
*/			
        ...
        $moduleUtil->getModuleData('modifyAdminMenu');
            ...
            if (auth()->user()->can('backup')) {
              ...
              </svg>', 'active' => request()->segment(1) == 'backup'])->order(100);
            }
            ...
            </svg>', 'active' => request()->segment(1) == 'manage-modules'])->order(100);
            }

            //Timesheet Management menu
            $menu->dropdown(
                'Timesheet Management',
                function ($sub) {
                    $sub->url(
                        route('timesheet.employees.index'),
                        'Employees',
                        ['icon' => '', 'active' => request()->segment(1) == 'timesheet-management' && request()->segment(2) == 'employees']
                    );
                    $sub->url(
                        route('timesheet.timesheets.index'),
                        'Timesheets',
                        ['icon' => '', 'active' => request()->segment(1) == 'timesheet-management' && request()->segment(2) == 'timesheets']
                    );
                },
                ['icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M3 12a9 9 0 1 0 18 0a9 9 0 0 0 -18 0" /><path d="M12 7v5l3 3" /></svg>']
            )->order(60);
