...
public function getRatetDetails()
{ 
    try {
        $data = DB::table('rates')
                    ->select('rates.rate as dailyrate')
                    ->orderByDesc('created_at')
                    ->orderByDesc('updated_at')
                    ->first();

        if ($data && isset($data->dailyrate)) {
            return $data;
        }

        // Return empty object with null dailyrate if no data found
        return (object) ['dailyrate' => null];
    } catch (\Exception $e) {
        \Log::error('Error fetching rate details: ' . $e->getMessage());
        return (object) ['dailyrate' => null];
    }
}
