...
@section('title')
    {{ trans_choice('accounting::lang.add', 1) }} {{ trans_choice('accounting::lang.account', 1) }}
@endsection
    ...
    <script>
        var app = new Vue({
            ...
            data: {
                ...
                gl_code: "{{ old('gl_code') }}",
                allow_manual: "{{ old('allow_manual', 1) }}",
                active: "{{ old('active', 1) }}",
                notes: "{{ old('notes') }}",
                currencies: {!! json_encode($currencies) !!},
                currency_id: "{{ old('currency_id') }}",
                opening_balance: "{{ old('opening_balance') }}",
                has_opening_balance: {{ old('has_opening_balance') ? 1 : 0 }},
